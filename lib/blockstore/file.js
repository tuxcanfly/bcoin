...
const {BlockRecord, TxRecord, FileRecord} = require('./records');
...
class FileBlockStore extends AbstractBlockStore {
  ...
  /**
   * This method indexes tx data in files.
   * @param {Buffer} block - The tx hash
   * @param {Buffer} entry - TX block entry
   * @returns {Promise}
   */

  async writeMeta(block, entry) {
    const b = this.db.batch();
    ...
    for (let i = 0; i < count; i++) {
      const length = data.length - br.offset;
      const tx = TX.fromReader(br);
      const offset = br.offset - length;
      const hash = tx.hash();
  ...
  /**
   * This will check if a transaction is available.
   * @param {Buffer} hash - The tx hash
   * @returns {Promise}
   */

  async hasMeta(hash) {
    return await this.db.has(layout.t.encode(hash));
  }
