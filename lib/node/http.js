...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/coin/address/:address', async (req, res) => {
      ...
      enforce(!this.chain.options.neutrino, 'Cannot get coins in SPV mode.');
  ...
  initSockets() {
    ...
    pool.on('cfheaders', (hashes, type) => {
      ...
      for (const socket of sockets)
        socket.fire('cfheaders', hashes, type);
    });

    pool.on('cfilter', (hash, data, type) => {
      const sockets = this.channel('chain');
      ...
      for (const socket of sockets)
        socket.fire('cfilter', hash, data, type);
    });
