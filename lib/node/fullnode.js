...
class FullNode extends Node {
  ...
  async getCoinsByAddress(addrs) {
    ...
    const index = await this.indexer.getCoinsByAddress(addrs);
    const out = [];
  ...
  async getMetaByAddress(addrs) {
    ...
    if (this.indexer) {
      const index = await this.indexer.getMetaByAddress(addrs);
      return index.concat(mempool);
    }
  ...
  async getMeta(hash) {
    ...
    if (this.indexer)
        return this.indexer.getMeta(hash);
  ...
  async hasTX(hash) {
    ...
    if (this.indexer)
      return this.indexer.hasTX(hash);
