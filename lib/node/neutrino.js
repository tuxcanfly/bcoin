...
const {FilterChain} = require('../blockchain/filterchain');
...
/**
 * Neutrino Node
 * Create an neutrino node which only maintains
 * a filterchain, a pool, and an http server.
 * @alias module:node.NeutrinoNode
 * @extends Node
 */

class NeturinoNode extends Node {
  /**
   * Create Neturino node.
   * @constructor
  ...
  async open() {
    assert(!this.opened, 'NeturinoNode is already open.');
    this.opened = true;
  ...
  async close() {
    assert(this.opened, 'NeturinoNode is not open.');
    this.opened = false;
  ...
  /* TODO: Scan for missed transactions */
