...
LowlevelUp.prototype.getBlock = async function getBlock(key) {
  return await this.db.getBlock(key);
};

LowlevelUp.prototype.putBlock = function putBlock(key, value) {
  if (!value)
    value = LOW;
  ...
  return new Promise((resolve, reject) => {
    ...
    this.db.putBlock(key, value, co.wrap(resolve, reject));
  });
};

LowlevelUp.prototype.removeBlock = function removeBlock(key) {
  return new Promise((resolve, reject) => {
    if (!this.loaded) {
      reject(new Error('Database is closed.'));
      return;
    }
    this.db.removeBlock(key, co.wrap(resolve, reject));
  });
};

LowlevelUp.prototype.sync = async function sync(key) {
  return await this.db.sync(key);
};
