...
function FlatFileDB(location) {
  ...
  this.cacheSize = 0;
  this.maxCacheSize = 128 * 1024; // 128 KiB
  this.lastFlush = Date.now();
  this.flushInterval = 1000 * 5; // 5s
}
...
FlatFileDB.prototype.commit = function commit() {
  ...
  while (!idel.current.isNull()) {
    this.index.del(idel.current.key, () => {});
    idel.next();
  }

  const iadd = this.add.iterator();
  while (!iadd.current.isNull()) {
    this.index.put(iadd.current.key, iadd.current.value, () => {});
    iadd.next();
  }
...
FlatFileDB.prototype.get = function get(key, options, callback) {
  ...
  if (!value) {
    this.index.get(key, options, (err, value) => {
