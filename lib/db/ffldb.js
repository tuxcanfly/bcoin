...
function DBCache(location) {
  ...
  this.height = 0;
  this.file = path.join(this.location, 'blocks', this.height + '.fdb');
  this.position = 0;
...
DBCache.prototype.get = function get(key, options, callback) {
  if (key[0] === 0x62) {
    this.db.get(key, options, (e, value) => {
      ...
      // TODO: write pos, len when inserting block
      // fetch and pass them here
...
DBCache.prototype.storeBlock = function storeBlock(fd, buffer) {
...
DBCache.prototype.fetchBlock = function fetchBlock(value, options, callback) {
  // TODO: fetch from cache if available
  fs.open(this.file, 'r', (err, fd) => {
    if (err) {
      throw err;
    }
    fs.read(fd, null, value.position, value.length, null,
      (err, pos, buf) => {
        if (err) {
          throw err;
        }
...
DBCache.prototype.commit = function commit() {
  ...
  fs.open(this.file, 'a', (err, fd) => {
    ...
    while(iter3.next()) {
      this.storeBlock(fd, iter3.value);
    }
    fs.close(fd, (err) => {
      if (err)
        throw err;
...
FlatFileDB.prototype.open = function open(options, callback) {
  ...
  fs.exists(path.join(this.location, 'blocks'), (exists) => {
    if (!exists)
      fs.mkdir(path.join(this.location, 'blocks'), (err) => {
        if (err)
          throw err;
      });
  });
