...
FlatFileDB.prototype.sync = async function sync() {
  for (const hash in this.hashes) {
    const i = this.hashes[hash];
    try {
      const loc = await this.blockio.writeBlock(this.blocks[i]);
...
FlatFileDB.prototype.get = function get(key, options, callback) {
  ...
  if (!value) {
    this.index.get(key, options, (value) => {
...
FlatFileDB.prototype.getBlock = async function getBlock(key) {
  ...
  const block = this.blockio.readBlock(loc);
  return block;
};

FlatFileDB.prototype.putBlock = function putBlock(key, value) {
  this.hashes[key.toString('hex')] = this.blocks.push(value) - 1;
  this.size += key.length;
  this.size += value.length;
...
FlatFileDB.prototype.removeBlock = function removeBlock(key) {
  delete this.blocks[this.hashes[key.toString('hex')]];
