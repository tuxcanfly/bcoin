...
function FlatFileDB(location) {
  ...
  this.index = LevelDOWN.call(this, location);
  ...
  this.dir = path.join(this.location, '..', 'blocks');
  this.file = path.join(this.dir, '000000000.fdb');
...
FlatFileDB.prototype.commit = function commit() {
  ...
  while (idel.next())
    this.index.del(idel.key, () => {
    });

  const iadd = this.add.iterator();
  while (iadd.next())
    this.index.put(iadd.key, iadd.value, () => {
    });
...
FlatFileDB.prototype.get = function get(key, options, callback) {
  ...
  if (!value) {
    this.index.get(key, options, callback);
  }
...
FlatFileDB.prototype.getBlock = function getBlock(key, callback) {
  if (key in this.hashes) {
    const block = this.blocks[this.hashes[key]];
    setImmediate(() => callback(null, block));
  }

  // TODO: only reads genesis block right now :/
  const fd = fs.openSync(this.file, 'r');
  const block = Buffer.alloc(285);
  const len = fs.readSync(fd, block, 0, block.length, 0);
  assert(block.length === len);
  fs.closeSync(fd);
  setImmediate(() => callback(null, block));
};
...
Iterator.prototype.init = function init() {
  const snapshot = this.db.add.snapshot();
  const iter = this.db.add.iterator(snapshot);
