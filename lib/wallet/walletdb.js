...
class WalletDB extends EventEmitter {
  ...
  constructor(options) {
    ...
    // Lock for all neutrino related stuff.
    this.cfLock = new Lock();
  ...
  _bind() {
    ...
    this.client.bind('cfheaders', async (hashes, type) => {
      try {
        await this.addCFHeaders(hashes, type);
      } catch (e) {
        this.emit('error', e);
      }
    });

    this.client.bind('cfilter', async (hash, data, type) => {
      try {
        await this.addCFilter(hash, data, type);
      } catch (e) {
        this.emit('error', e);
      }
    });
  ...
  async addCFHeaders(hashes, type) {
    const unlock = await this.cfLock.lock();
    try {
      return await this._addCFHeaders(hashes, type);
    } finally {
      unlock();
    }
  }

  async _addCFHeaders(hashes, type) {
    const b = this.db.batch();
    const pair = type ? layout.X : layout.G;
    for (const hash of hashes) {
      b.put(pair.build(), hash);
    }
    return b.write();
  }

  async addCFilter(hash, data, type) {
    const unlock = await this.cfLock.lock();
    try {
      return await this._addCFilter(hash, data, type);
    } finally {
      unlock();
    }
  }

  async _addCFilter(hash, data, type) {
    const b = this.db.batch();
    const pair = type ? layout.x : layout.g;
    b.put(pair.build(), hash);
    return b.write();
