...
class Pool extends EventEmitter {
  ...
  async handleGetCFHeaders(peer, packet) {
    ...
    while (entry) {
      const cfheader = await this.chain.getCFHeader(
  ...
  /**
   * Handle `cfheaders` packet from a given peer.
   * @method
   * @private
   * @param {Peer} peer
  ...
  async handleCFHeaders(peer, packet) {
    const unlock = await this.locker.lock();
    try {
      return await this._handleCFHeaders(peer, packet);
    } finally {
      unlock();
    }
  }
  ...
  async _handleCFHeaders(peer, packet) {
    // TODO: write header batch to db

    if (!this.options.neutrino) {
      this.logger.warning(
        'Peer sent unsolicited cfilter (%s).',
        peer.hostname());
      return;
    }

    const cfheaders = packet.hashes;
    const type = packet.filterType;

    this.logger.debug(
      'Received %s type %s cfheaders from peer (%s).',
      cfheaders.length, type,
      peer.hostname());

    await this.addCFHeaders(peer, packet.hashes, packet.filterType);
  }
  ...
  async handleGetCFilter(peer, packet) {
    ...
    const cfilter = await this.chain.getCFilter(
      packet.hash,
      packet.filterType
    );
  ...
  async handleCFilter(peer, packet) {
    ...
    await this.addCFilter(peer, packet.hash, packet.data, packet.filterType);
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    if (this.options.neutrino)
      await this.getCFHeaders(this.headerTip.hash, node.hash);
  ...
  /**
   * Attempt to add cfheaders to chain.
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFHeaders} hashes
   * @returns {Promise}
   */

  async addCFHeaders(peer, hashes, stop, filterType) {
    const hash = stop;
    const unlock = await this.locker.lock(hash);
    try {
      return await this._addCFHeader(peer, hashes, filterType);
    } finally {
      unlock();
    }
  }
  ...
  /**
   * Attempt to add cfheaders to chain (without a lock).
   * @method
   * @private
   * @param {Peer} peer
   * @param {CFHeaders} hashes
  ...
  async _addCFHeader(peer, hashes, filterType) {
    if (!this.syncing)
      return;

    let entry;
    try {
      entry = await this.chain.addCFHeaders(hashes, filterType);
    } catch (err) {
      throw err;
    }

    this.logger.info(
      'Saved %s cfheaders from (%s).', entry,
      peer.hostname());
  ...
  /**
   * Attempt to add cfilter to chain.
   * @method
   * @private
   * @param {Peer} peer
   * @param {Hash} hash
  ...
  async addCFilter(peer, hash, cfilter, filterType) {
    const unlock = await this.locker.lock(hash);
    try {
      return await this._addCFilter(peer, hash, cfilter, filterType);
    } finally {
      unlock();
    }
  }
  ...
  /**
   * Attempt to add cfilter to chain (without a lock).
   * @method
   * @private
   * @param {Peer} peer
   * @param {Hash} hash
  ...
  async _addCFilter(peer, hash, cfilter, filterType) {
    if (!this.syncing)
      return;

    let entry;
    try {
      entry = await this.chain.addCFilter(hash, cfilter, filterType);
    } catch (err) {
      throw err;
    }

    this.logger.info(
      'Saved %s cfilter from (%s).', entry,
      peer.hostname());
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.neutrino != null) {
      ...
      assert(options.neutrino === this.chain.options.neutrino);
      this.neutrino = options.neutrino;
    } else {
      this.neutrino = this.chain.options.neutrino;
    }
    ...
    if (options.cf != null) {
      assert(typeof options.cf === 'boolean');
      this.cf = options.cf;
    }
    ...
    if (this.neutrino) {
      this.requiredServices |= common.services.CF;
