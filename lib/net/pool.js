...
class Pool extends EventEmitter {
  ...
  resolveHeaders(peer) {
    ...
    for (let node = this.headerNext; node; node = node.next) {
      ...
      if (this.cfOutpoints.length === 0)
        continue;

      if (!this.cfilterMap.has(node.hash))
        continue; // TODO: raise error

      const hash = Buffer.from(node.hash, 'hex');
      const key = hash.slice(0, 16);
      const raw = this.cfilterMap.get(node.hash);

      const cfilter = GCSFilter.fromRaw(raw);
      if (cfilter.matchAny(key, this.cfOutpoints))
        items.push(node.hash);
    }
  ...
  async _handleCFHeaders(peer, packet) {
    ...
    for (const cfheader in cfheaders)
      this.cfheaderChain.push(cfheader);
  }
  ...
  async handleCFilter(peer, packet) {
    ...
    this.cfilterMap.set(packet.hash, packet.data);
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    if (this.options.neutrino) {
      ...
      peer.sendGetCFilter(node.hash, common.FILTERS.REGULAR);
    }
  ...
  watchCFOutpoints(data) {
    this.cfOutpoints.push(data);
  }
