...
class Pool extends EventEmitter {
  ...
  resetChain() {
    ...
    if (this.options.neutrino || tip.height < this.network.lastCheckpoint) {
  ...
  async handleGetCFHeaders(peer, packet) {
    ...
    while (entry) {
      const cfheader = await this.chain.getCFHeader(entry.hash);
  ...
  async _handleCFHeaders(peer, packet) {
    ...
    assert(this.cfheaderChain.size > 0);

    let node = null;

    for (const header of headers) {
      const last = this.cfheaderChain.tail;
      const hash = header.hash('hex');
      const height = last.height + 1;

      if (!header.verify()) {
        this.logger.warning(
          'Peer sent an invalid cfheader (%s).',
          peer.hostname());
        peer.increaseBan(100);
        peer.destroy();
        return;
      ...
      if (header.prevBlock !== last.hash) {
        this.logger.warning(
          'Peer sent a bad cfheader chain (%s).',
          peer.hostname());
        peer.destroy();
        return;
      }
      ...
      this.cfheaderChain.push(node);
    }

    this.logger.debug(
      'Received %s cfheaders from peer (%s).',
      headers.length,
      peer.hostname());

    // Request more headers.
    peer.sendGetCFilters([node.hash], null);
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    // Request more headers.
    peer.sendGetHeaders([node.hash], null);
  }
