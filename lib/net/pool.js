...
class Pool extends EventEmitter {
  ...
  resolveHeaders(peer) {
    ...
    for (let node = this.headerNext; node; node = node.next) {
      ...
      if (cfilter.matchAny(key, this.watching))
        items.push(node.hash);
    }
  ...
  async _handleCFHeaders(peer, packet) {
    ...
    for (const cfheader of cfheaders) {
      this.filterChain.push(cfheader);
    }
  }
  ...
  async _handleHeaders(peer, packet) {
    ...
    for (const header of headers) {
      ...
      if (this.options.neutrino) {
        ...
        peer.sendGetCFilter(node.hash, common.FILTERS.EXTENDED);
      }
    }
