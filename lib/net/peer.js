...
class Peer extends EventEmitter {
  ...
  /**
   * Send `getcfheaders` to peer. Note that unlike
   * `getblocks`, `getcfheaders` can have a null locator.
   * @param {Hash[]?} locator - Chain locator.
   * @param {Hash?} stop - Hash to stop at.
   * @param {Boolean?} extended - Filter type.
   */

  sendGetCFHeaders(locator, stop, extended) {
    const packet = new packets.GetCFHeadersPacket(locator, stop, extended);
  ...
  sendGetCFHeaders(locator, stop) {
    const packet = new packets.GetCFHeadersPacket(locator, stop);
