...
/**
 * Represents a `getcfilter` packet.
 * @constructor
 * @param {Hash} block hash
 * @property {Boolean} extended
 */

function GetCFilterPacket(hash, extended) {
  if (!(this instanceof GetCFilterPacket))
    return new GetCFilterPacket(hash, extended);
...
Object.setPrototypeOf(GetCFilterPacket.prototype, Packet.prototype);

GetCFilterPacket.prototype.cmd = 'getcfilter';
GetCFilterPacket.prototype.type = exports.types.GETCFILTER;
...
GetCFilterPacket.prototype.getSize = function getSize() {
  return 33;
};
...
GetCFilterPacket.prototype.toWriter = function toWriter(bw) {
  bw.writeHash(this.hash || encoding.ZERO_HASH);
  bw.writeU8(this.extended ? 0 : 1);
...
GetCFilterPacket.prototype.toRaw = function toRaw() {
  const size = this.getSize();
  return this.toWriter(new StaticWriter(size)).render();
};
...
GetCFilterPacket.prototype.fromReader = function fromReader(br) {
  this.hash = br.readHash('hex');
  this.extended = br.readU8() === 0;
...
GetCFilterPacket.prototype.fromRaw = function fromRaw(data) {
  return this.fromReader(new BufferReader(data));
};
...
GetCFilterPacket.fromReader = function fromReader(br) {
  return new GetCFilterPacket().fromReader(br);
};
...
GetCFilterPacket.fromRaw = function fromRaw(data, enc) {
  if (typeof data === 'string')
    data = Buffer.from(data, enc);
  return new GetCFilterPacket().fromRaw(data);
};

/**
 * Represents a `cfilter` packet.
 * @constructor
 * @param {Hash} block hash
 * @property {Boolean} extended
...
function CFilterPacket(hash, extended, data) {
  if (!(this instanceof CFilterPacket))
    return new CFilterPacket(hash, extended, data);
...
Object.setPrototypeOf(CFilterPacket.prototype, Packet.prototype);

CFilterPacket.prototype.cmd = 'cfilter';
CFilterPacket.prototype.type = exports.types.CFILTER;
...
CFilterPacket.prototype.Size = function Size() {
  let size = 0;
  size += 33;
...
CFilterPacket.prototype.toWriter = function toWriter(bw) {
  bw.writeHash(this.hash || encoding.ZERO_HASH);
  bw.writeU8(this.extended ? 0 : 1);
...
CFilterPacket.prototype.toRaw = function toRaw() {
...
CFilterPacket.prototype.fromReader = function fromReader(br) {
  this.hash = br.readHash('hex');
  this.extended = br.readU8() === 0;
...
CFilterPacket.prototype.fromRaw = function fromRaw(data) {
  return this.fromReader(new BufferReader(data));
};
...
CFilterPacket.fromReader = function fromReader(br) {
  return new CFilterPacket().fromReader(br);
};
...
CFilterPacket.fromRaw = function fromRaw(data, enc) {
  if (typeof data === 'string')
    data = Buffer.from(data, enc);
  return new CFilterPacket().fromRaw(data);
};

/**
 * Represents a `getcfheaders` packet.
 * @constructor
 * @param {Hash[]} locator
 * @param {Hash?} stop
 * @property {Boolean} extended
...
function GetCFHeadersPacket(locator, stop, extended) {
  if (!(this instanceof GetCFHeadersPacket))
    return new GetCFHeadersPacket(locator, stop, extended);
...
Object.setPrototypeOf(GetCFHeadersPacket.prototype, Packet.prototype);

GetCFHeadersPacket.prototype.cmd = 'getcfheaders';
GetCFHeadersPacket.prototype.type = exports.types.GETCFHEADERS;
...
GetCFHeadersPacket.prototype.getSize = function getSize() {
  let size = 0;
  size += 4;
  size += encoding.sizeVarint(this.locator.length);
  size += 32 * this.locator.length;
...
GetCFHeadersPacket.prototype.toWriter = function toWriter(bw) {
  assert(this.locator.length <= 50000, 'Too many cfheader hashes.');
...
GetCFHeadersPacket.prototype.toRaw = function toRaw() {
  const size = this.getSize();
  return this.toWriter(new StaticWriter(size)).render();
};
...
GetCFHeadersPacket.prototype.fromReader = function fromReader(br) {
  this.version = br.readU32();

  const count = br.readVarint();

  assert(count <= 50000, 'Too many cfheader hashes.');
...
GetCFHeadersPacket.prototype.fromRaw = function fromRaw(data) {
  return this.fromReader(new BufferReader(data));
};
...
GetCFHeadersPacket.fromReader = function fromReader(br) {
  return new GetCFHeadersPacket().fromReader(br);
};
...
GetCFHeadersPacket.fromRaw = function fromRaw(data, enc) {
  if (typeof data === 'string')
    data = Buffer.from(data, enc);
  return new GetCFHeadersPacket().fromRaw(data);
};

/**
 * Represents a `cfheaders` packet.
 * @constructor
...
function CFHeadersPacket(stop, extended, hashes) {
  if (!(this instanceof CFHeadersPacket))
    return new CFHeadersPacket(stop, extended, hashes);
...
Object.setPrototypeOf(CFHeadersPacket.prototype, Packet.prototype);

CFHeadersPacket.prototype.cmd = 'cfheaders';
CFHeadersPacket.prototype.type = exports.types.CFHEADERS;
...
CFHeadersPacket.prototype.getSize = function getSize() {
  let size = 0;
  size += 33;
...
CFHeadersPacket.prototype.toWriter = function toWriter(bw) {
  assert(this.hashes.length <= 50000, 'Too many cfheader hashes.');
...
CFHeadersPacket.prototype.toRaw = function toRaw() {
  const size = this.getSize();
  return this.toWriter(new StaticWriter(size)).render();
};
...
CFHeadersPacket.prototype.fromReader = function fromReader(br) {
  this.stop = br.readHash('hex');
  this.extended = br.readU8() === 0;
...
CFHeadersPacket.prototype.fromRaw = function fromRaw(data) {
  return this.fromReader(new BufferReader(data));
};

/**
 * Instantiate cfheader packet from buffer reader.
 * @param {BufferReader} br
 * @returns {CFHeadersPacket}
 */

CFHeadersPacket.fromReader = function fromReader(br) {
  return new CFHeadersPacket().fromReader(br);
};

/**
 * Instantiate cfheader packet from serialized data.
 * @param {Buffer} data
 * @param {String?} enc
 * @returns {CFHeadersPacket}
 */

CFHeadersPacket.fromRaw = function fromRaw(data, enc) {
  if (typeof data === 'string')
    data = Buffer.from(data, enc);
  return new CFHeadersPacket().fromRaw(data);
};
