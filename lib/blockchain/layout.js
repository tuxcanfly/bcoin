 ...
 *   O -> chain options
 *   e[hash] -> entry
 *   g[hash] -> basic committed filter
 *   G[hash] -> basic committed filter header
 *   h[hash] -> height
 *   H[height] -> hash
 *   n[hash] -> next hash
 *   p[hash] -> tip index
 ...
 *   x[hash] -> extended committed filter
 *   X[hash] -> extended committed filter header
...
const layout = {
  ...
  g: function g(hash) {
    return pair(0x67, hash);
  },
  G: function G(hash) {
    return pair(0x47, hash);
  },
  h: function h(hash) {
  ...
  x: function x(hash) {
    return pair(0x78, hash);
  },
  X: function X(hash) {
    return pair(0x58, hash);
  },
