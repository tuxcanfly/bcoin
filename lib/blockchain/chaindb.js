...
class ChainDB {
  ...
  async saveGenesis() {
    ...
    if (this.options.indexFilters) {
      ...
      b.put(layout.G.build(hash), basic.header(consensus.ZERO_HASH));

      const extended = GCSFilter.fromExtended(block);
      b.put(layout.x.build(hash), extended.toRaw());
      b.put(layout.X.build(hash), extended.header(consensus.ZERO_HASH));
  ...
  async indexFilters(block, prevBasic, prevExt) {
    ...
    const extended = GCSFilter.fromExtended(block);
    b.put(layout.x.build(hash), extended.toRaw());
    b.put(layout.X.build(hash), extended.header(prevExt));
