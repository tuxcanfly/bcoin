...
ChainDB.prototype.saveFilterHeader = function saveFilterHeader(prev, extended, filter) {
  if (!this.options.indexFilters)
    return;
  ...
  const header = this.get(pair, prev);

  this.put(pair, filter.header(header));
};
...
ChainDB.prototype.saveFilters = function saveFilters(block, entry) {
  if (!this.options.indexFilters)
    return;
  ...
  this.saveFilterHeader(prev, false, basic);

  const extended = GCSFilter.fromExtended(block);
  this.put(layout.x(hash), extended);
  this.saveFilterHeader(prev, true, extended);
};
