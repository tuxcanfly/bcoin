...
class ChainDB {
  ...
  async getCFHeader(hash, type) {
    ...
    assert(cfheader, 'Missing cfheader.');
  ...
  async saveGenesis() {
    ...
    if (this.options.indexFilters) {
      await this.indexFilters(block, consensus.ZERO_HASH);
    }
  ...
  async connectBlock(entry, block, view) {
    ...
    if (this.options.indexFilters) {
      ...
      const prevBasic = this.getCFHeader(
        Buffer.from(block.prevBlock, 'hex'),
        common.FILTERS.REGULAR
      );
      const prevExt = this.getCFHeader(
        Buffer.from(block.prevBlock, 'hex'),
  ...
  async indexFilters(block, prevBasic, prevExt) {
    ...
    const b = this.db.batch();
    b.put(layout.g.build(hash), basic.data);
    b.put(layout.G.build(hash), prevBasic);

    if (prevExt) {
      const extended = GCSFilter.fromExtended(block);
      b.put(layout.X.build(hash), prevExt);
      b.put(layout.x.build(hash), extended.data);
    }
