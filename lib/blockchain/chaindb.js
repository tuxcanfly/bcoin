...
class ChainDB {
  ...
  async verifyFlags(state) {
    ...
    if (options.indexTX && !flags.indexTX) {
      await this.rebuildIndex(this.indexTX);
      needsSave = true;
    }

    if (!options.indexTX && flags.indexTX) {
      await this.rebuildIndex(this.unindexTX);
      needsSave = true;
    }
  ...
  /**
   * Rebuild index
   * @param {Function} indexing function
   * @returns {Promise}
   */

  async rebuildIndex(func) {
    this.start();
    try {
      await this._rebuildIndex(func);
    } catch (e) {
      this.drop();
      throw e;
    }
  ...
  async updateInputs(block, prev, txs) {
    const view = new CoinView();
  ...
  async _rebuildIndex(func) {
    const filter = new BloomFilter();

    await this.scan(null, filter, async (entry, txs) => {
      let view = new CoinView();
      if (!entry.isGenesis()) {
        const prev = await this.getPrevious(entry);
        assert(prev);
        view = await this.updateInputs(entry, prev, txs);
      }

      for (let i = 0; i < txs.length; i++) {
        func.call(this, txs[i], view, entry, i);
      }
    });
  }
