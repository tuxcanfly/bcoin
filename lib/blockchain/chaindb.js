...
class ChainDB {
  ...
  async getCFilter(hash, type) {
    ...
    const pair = type ? layout.x : layout.g;
    const raw = await this.db.get(pair.build(hash));
    assert(raw, `Missing cfilter ${hash.toString('hex')} ${type}.`);
    const cfilter = GCSFilter.fromRaw(raw);

    return cfilter.toNBytes();
  }
  ...
  async saveGenesis() {
    ...
    if (this.options.indexFilters) {
      ...
      b.put(layout.g.build(hash), basic.toRaw());
      b.put(layout.G.build(hash), basic.header(consensus.ZERO_HASH).reverse());

      const extHeader = hash256.root(consensus.ZERO_HASH, consensus.ZERO_HASH);
      b.put(layout.X.build(hash), extHeader.reverse());
  ...
  async verifyFlags(state) {
    ...
    if (options.neutrino && !flags.neutrino)
      throw new Error('Cannot retroactively enable Neutrino.');
  ...
  async connectBlock(entry, block, view) {
    ...
    if (this.options.indexFilters) {
      ...
      await this.indexFilters(block, prevBasic, prevExt);
    }
  ...
  /**
   * Remove committed filter for block.
   * @private
   * @param {Block} block
   */
  ...
  /**
   * Save committed filter for block.
   * @private
   * @param {Block} block
   * @param {Hash} prev filter header - regular
   * @param {Hash} prev filter header - extended
   */

  async indexFilters(block, prevBasic, prevExt) {
    if (!this.options.indexFilters)
      return;
    ...
    const basic = GCSFilter.fromBlock(block);
    b.put(layout.g.build(hash), basic.toRaw());
    b.put(layout.G.build(hash), basic.header(prevBasic).reverse());

    const extended = GCSFilter.fromBlock(block);
    if (extended.data.length !== 0) {
      b.put(layout.x.build(hash), extended.toRaw());
      b.put(layout.X.build(hash), extended.header(prevExt).reverse());
    } else {
      b.put(layout.X.build(hash),
        hash256.root(consensus.ZERO_HASH, prevExt).reverse());
    }
