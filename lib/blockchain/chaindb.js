...
class ChainDB {
  ...
  async saveGenesis() {
    ...
    if (this.options.indexFilters) {
      this.put(layout.G(block.prevBlock), consensus.NULL_HASH);
      this.put(layout.X(block.prevBlock), consensus.NULL_HASH);
    }
  ...
  writeDeployments(b) {
    const bw = bio.static(1 + 17 * this.network.deploys.length);
  ...
  /**
   * Save filter header for block.
   * @private
   * @param {Hash} block hash
   * @param {Hash} prev block hash
   * @param {Bool} extended
   * @param {Filter} filter
   */

  indexFilterHeader(hash, prev, extended, filter) {
    if (!this.options.indexFilters)
      return;

    filter = filter || consensus.NULL_HASH;
    extended = extended || false;

    const pair = extended ? layout.X : layout.G;
    const header = this.db.get(pair(prev));

    this.put(pair(hash), filter.header(header));
  }
  ...
  indexFilters(block, entry) {
    if (!this.options.indexFilters)
      return;

    const hash = block.hash();
    const prev = entry.prevBlock;

    const basic = GCSFilter.fromBlock(block);
    this.put(layout.g(hash), basic);
    this.indexFilterHeader(hash, prev, false, basic);

    const extended = GCSFilter.fromExtended(block);
    this.put(layout.x(hash), extended);
    this.indexFilterHeader(hash, prev, true, extended);
  }
