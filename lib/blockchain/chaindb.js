...
class ChainDB {
  ...
  /**
   * Retrieve commited filter by hash and type..
  ...
  /**
   * Retrieve commited filter header by hash and type..
   * @returns {Promise} - Returns {@link Hash}.
   */
  ...
  async verifyFlags(state) {
    ...
    if (!options.neutrino && flags.neutrino)
      throw new Error('Cannot retroactively disable neutrino.');
  ...
  async scan(start, filter, iter) {
    ...
    while (entry) {
      ...
      if (!block) {
        if (!this.options.spv && !this.neutrino && !this.options.prune)
...
class ChainFlags {
  ...
  fromRaw(data) {
    ...
    this.indexFilters = (flags & 32) !== 0;
    this.bip91 = (flags & 64) !== 0;
    this.bip148 = (flags & 128) !== 0;
    this.neutrino = (flags & 256) !== 0;
