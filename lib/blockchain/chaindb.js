...
ChainDB.prototype.saveGenesis = async function saveGenesis() {
  ...
  if (this.options.indexFilters) {
    this.put(layout.G(block.prevBlock), encoding.ZERO_HASH);
    this.put(layout.X(block.prevBlock), encoding.ZERO_HASH);
  }
...
ChainDB.prototype.indexFilterHeader = function indexFilterHeader(hash, prev, extended, filter) {
  if (!this.options.indexFilters)
    return;

  filter = filter || encoding.ZERO_HASH;
...
ChainDB.prototype.indexFilters = function indexFilters(block, entry) {
  if (!this.options.indexFilters)
    return;
...
ChainDB.prototype.unindexFilters = function unindexFilters(block) {
  if (!this.options.indexFilters)
    return;
