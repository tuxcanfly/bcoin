...
class ChainDB {
  ...
  async saveGenesis() {
    ...
    if (this.options.indexFilters) {
      ...
      await this.indexFilters(block, consensus.ZERO_HASH, consensus.ZERO_HASH);
    }
  ...
  async indexFilters(block, prevBasic, prevExt) {
    ...
    b.put(layout.x.build(hash), ext.toRaw());
    b.put(layout.X.build(hash), ext.header(prevExt).reverse());
