...
class Chain extends AsyncEmitter {
  ...
  async verifyContext(block, prev, flags) {
    ...
    // Skip everything if we're in lite cient mode.
    if (!this.fullnode) {
  ...
  /**
   * Reorganize the blockchain for Neutrino. This
   * will reset the chain to the fork block.
   * @private
   * @param {ChainEntry} competitor - The competing chain's tip.
   * @returns {Promise}
   * TODO: update for neutrino
   */

  async reorganizeNeutrino(competitor) {
    const tip = this.tip;
    const fork = await this.findFork(tip, competitor);
    ...
    this.logger.warning(
      'Neutrino reorganization: old=%s(%d) new=%s(%d)',
      tip.rhash(),
      tip.height,
      competitor.rhash(),
      competitor.height
  ...
  async setBestChain(entry, block, prev, flags) {
    ...
    if (entry.prevBlock !== this.tip.hash) {
      ...
      if (this.options.neutrino) {
        await this.reorganizeNeutrino(entry);
        return;
      }
...
class ChainOptions {
  ...
  fromOptions(options) {
    ...
    if (options.prefix != null) {
      ...
      switch (options) {
        case options.spv:
          this.location = path.join(this.prefix, 'spvchain');
          break;
        case options.neutrino:
          this.location = path.join(this.prefix, 'neutrinochain');
          break;
        default:
          this.location = path.join(this.prefix, 'chain');
      }
    }
