...
class Indexer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger = this.options.logger.context('indexer');
  ...
  async open() {
    await this.db.open();
    await this.db.verify(this.layout.V.build(), 'index', 7);
  ...
  async verifyNetwork() {
    const raw = await this.db.get(this.layout.O.build());

    if (!raw) {
      const b = this.db.batch();
      b.put(this.layout.O.build(), fromU32(this.network.magic));
      return b.write();
    }
  ...
  async syncState() {
    ...
    for (let height = 0; height < hashes.length; height++) {
      ...
      b.put(this.layout.h.build(height), meta.toHash());
      tip = meta;
    }
    ...
    b.put(this.layout.R.build(), state.toRaw());
  ...
  async getState() {
    const data = await this.db.get(this.layout.R.build());
  ...
  async setTip(tip) {
    ...
    if (tip.height < state.height) {
      ...
      while (state.height !== tip.height) {
        b.del(this.layout.h.build(state.height));
        state.height -= 1;
      }
    ...
    // Save tip and state.
    b.put(this.layout.h.build(tip.height), tip.toHash());
    b.put(this.layout.R.build(), state.toRaw());
  ...
  async getBlock(height) {
    const data = await this.db.get(this.layout.h.build(height));
  ...
  async rollback(height) {
    ...
    // TODO: Cleanup - delete obsolote indexes
